/* ============================================================
   ...
   Premium (vive dentro de la sección de proyecciones).
   ============================================================ */
...
/* Config por deporte: etiqueta del stat, umbral de "acierto", verbo. */
const CFG = {
  mlb:    { stat: () => L('Hits', 'Hits'),   umbral: 1,  unidad: '',      hit: (v) => v >= 1,  logro: () => L('games with a hit', 'partidos con hit') },
  soccer: { stat: () => L('Goals', 'Goles'), umbral: 1,  unidad: '',      hit: (v) => v >= 1,  logro: () => L('games scoring', 'partidos con gol') },
  nba:    { stat: () => L('Points', 'Puntos'), umbral: 20, unidad: '',    hit: (v) => v >= 20, logro: () => L('games with 20+', 'partidos con 20+') },
};
...
/* ---- Historial real (últimos 10) por deporte ---- */
...
async function logESPN(sport, id, campo) {
  // sport: 'basketball/nba' | 'soccer'
  const d = await pedir(`https://site.web.api.espn.com/apis/common/v3/sports/${sport}/athletes/${id}/gamelog`);
  const vals = [];
  const eventos = d?.events ? Object.values(d.events) : [];
  // Estructura: seasonTypes[].categories[].events[].stats[] — parser defensivo
  const filas = [];
  ...
  filas.slice(0, 10).forEach(ev => {
    const st = ev.stats || [];
    let v = idx >= 0 ? num(st[idx]) : null;
    if (v == null) v = num(st.find && st.find(x => false));
...
function sintetizar(sport, jugador) {
  ...
  if (sport === 'nba') {
    ...
    const lam = -Math.log(1 - p);   // λ que reproduce P(≥1)=p
...
function inyectarCSS() {
  ...
  st.textContent = `
  .trk-bg{position:fixed;inset:0;z-index:120;background:rgba(3,6,11,.72);backdrop-filter:blur(4px);
    display:flex;align-items:center;justify-content:center;padding:18px;opacity:0;transition:opacity .18s}
  .trk-bg.on{opacity:1}
  .trk{width:100%;max-width:560px;background:linear-gradient(180deg,#121a26,#0d131d);border:1px solid rgba(255,255,255,.1);
    border-radius:18px;overflow:hidden;box-shadow:0 30px 80px rgba(0,0,0,.6);transform:translateY(8px);transition:transform .18s}
  .trk-bg.on .trk{transform:none}
  .trk-hd{position:relative;padding:20px 22px;border-bottom:1px solid rgba(255,255,255,.07);
    background:linear-gradient(135deg,rgba(56,169,240,.14),rgba(240,53,58,.10))}
  .trk-eq{font-family:"Chakra Petch",sans-serif;font-weight:700;font-size:11px;letter-spacing:.18em;text-transform:uppercase;color:#9fb6cf}
  .trk-nm{font-family:"Chakra Petch",sans-serif;font-weight:800;font-size:23px;color:#fff;margin-top:3px}
  .trk-x{position:absolute;top:16px;right:16px;width:32px;height:32px;border-radius:9px;border:1px solid rgba(255,255,255,.14);
    background:rgba(0,0,0,.3);color:#cdd6e2;cursor:pointer;font-size:18px;line-height:1;display:grid;place-items:center}
  .trk-x:hover{color:#fff;border-color:rgba(255,255,255,.3)}
  .trk-bd{padding:20px 22px 24px}
  .trk-sum{display:flex;gap:12px;flex-wrap:wrap;margin-bottom:18px}
  ...
  .trk-kpi .k{font-size:10px;letter-spacing:.12em;text-transform:uppercase;color:#5c6879;font-weight:700}
  .trk-kpi .v{font-family:"Chakra Petch",sans-serif;font-weight:800;font-size:26px;color:#4db4f7;margin-top:3px}
  .trk-kpi .v small{font-size:14px;color:#98a4b4;font-weight:700}
  .trk-chart{position:relative;height:150px;display:flex;align-items:flex-end;gap:6px;padding:14px 0 6px;
    border-bottom:1px solid rgba(255,255,255,.1)}
  .trk-col{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:flex-end;height:100%;gap:5px}
  .trk-bar{width:100%;max-width:26px;border-radius:5px 5px 0 0;min-height:3px;transition:height .5s cubic-bezier(.2,.7,.2,1)}
  .trk-bar.hit{background:linear-gradient(180deg,#4db4f7,#1f8fe0);box-shadow:0 0 10px rgba(77,180,247,.4)}
  ...
  .trk-line{position:absolute;left:0;right:0;border-top:1px dashed rgba(244,73,78,.55);pointer-events:none}
  .trk-line span{position:absolute;right:0;top:-8px;font-size:9px;font-weight:800;color:#f6a7aa;background:#0d131d;padding:0 5px;letter-spacing:.05em}
  .trk-axis{display:flex;gap:6px;margin-top:6px}
  .trk-axis .g{flex:1;text-align:center;font-size:9px;color:#5c6879;font-weight:600}
  .trk-note{margin-top:16px;font-size:12.5px;color:#98a4b4;line-height:1.5}
  .trk-note b{color:#cdd6e2}
  .trk-demo{display:inline-block;margin-top:12px;font-size:10px;font-weight:800;letter-spacing:.1em;text-transform:uppercase;
    color:#f6d38c;background:rgba(243,177,61,.1);border:1px solid rgba(243,177,61,.3);border-radius:6px;padding:3px 8px}
  `;
  document.head.appendChild(st);
}

function chartHTML(vals, cfg) {
  const max = Math.max(cfg.umbral, ...vals, 1);
  const cols = vals.map((v, i) => {
    const h = Math.round((v / max) * 100);
    const hit = cfg.hit(v);
    return `<div class="trk-col"><span class="trk-val">${v}</span>
      <div class="trk-bar ${hit ? 'hit' : 'miss'}" style="height:0" data-h="${h}"></div></div>`;
  }).join('');
  const linePct = 100 - Math.round((cfg.umbral / max) * 100);
  const axis = vals.map((_, i) => `<div class="g">${i === vals.length - 1 ? L('Last', 'Últ.') : '-' + (vals.length - 1 - i)}</div>`).join('');
  return `<div class="trk-chart">
      <div class="trk-line" style="top:calc(14px + ${linePct}% * (150px - 20px) / 100)"><span>${cfg.umbral}${cfg.umbral === 20 ? '' : '+'}</span></div>
      ${cols}
...
export async function abrirTracker(jugador, sport) {
  ...
  bg.innerHTML = `<div class="trk" role="dialog" aria-modal="true">
    <div class="trk-hd">
      <button class="trk-x" aria-label="Close">×</button>
  ...
  document.addEventListener('keydown', function esc2(e) { if (e.key === 'Escape') { cerrar(); document.removeEventListener('keydown', esc2); } });

  // Obtener trayectoria: real si hay id, si no demo
  ...
  if (!vals || !vals.length || vals.every(v => v === 0 && sport !== 'mlb')) { vals = sintetizar(sport, jugador); demo = true; }
  vals = vals.slice(-10);
  ...
  const inner = bg.querySelector('#trk-inner');
  inner.innerHTML = `
    <div class="trk-sum">
  ...
  requestAnimationFrame(() => inner.querySelectorAll('.trk-bar').forEach(b => { b.style.height = b.dataset.h + '%'; }));
}
